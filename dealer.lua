...
if response then
  ...
  if success and type(playlist) == "table" then
    ...
    local function displayMusicMenu()
      ...
      local easterEggPlaylistURL = "https://raw.githubusercontent.com/Miniprimestaff/music-cc/main/program/playlistdark.json"
      ...
      while true do
        ...
        elseif key == keys.enter then
          ...
          secretCode = secretCode .. string.char(key)

          -- Vérifier si la chaîne secrète correspond au code caché (b-a-t-m-a-n)
          if secretCode == "b-a-t-m-a-n" then
            easterEggMode = true
            playlistURL = easterEggPlaylistURL
            response = http.get(playlistURL)
            if response then
              playlistData = response.readAll()
              response.close()
              success, playlist = pcall(textutils.unserializeJSON, playlistData)
              if success and type(playlist) == "table" then
                musicList = {}
                for _, entry in ipairs(playlist) do
                  table.insert(musicList, entry.title)
                ...
                currentPage = 1
                totalPages = math.ceil(totalOptions / itemsPerPage)
                selectedIndex = 1
                term.setBackgroundColor(colors.purple) -- Changer la couleur de fond en violet pour l'easter egg
              else
                print("Erreur de parsing du fichier de la playlist easter egg.")
              end
            else
              print("Erreur lors du téléchargement du fichier de la playlist easter egg.")
            end
          end
